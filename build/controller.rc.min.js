...
!this._reconnection||0!==this.attempts||(this.openReconnect=!0,this.reconnect())};b.prototype.open=b.prototype.connect=function(b){k("readyState %s",this.readyState);if(~this.readyState.indexOf("open"))return this;k("opening %s",this.uri);var e=this.engine=a(this.uri,this.opts),c=this;this.readyState="opening";var q=h(e,"open",function(){c.onopen();b&&b()}),s=h(e,"error",function(a){k("connect_error");c.cleanup();c.readyState="closed";c.emitAll("connect_error",a);if(b){var e=Error("Connection error");
e.data=a;b(e)}c.maybeReconnectOnOpen()});if(!1!==this._timeout){var d=this._timeout;k("connect attempt will timeout after %d",d);var z=setTimeout(function(){k("connect attempt timed out after %d",d);q.destroy();e.close();e.emit("error","timeout");c.emitAll("connect_timeout",d)},d);this.subs.push({destroy:function(){clearTimeout(z)}})}this.subs.push(q);this.subs.push(s);return this};b.prototype.onopen=function(){k("open");this.cleanup();this.readyState="open";this.emit("open");var a=this.engine;this.subs.push(h(a,
...
function(b){a()})},a=function(){gyro.frequency=200;gyro.startTracking(function(a){g("rc_accelerometer",a)})},e=function(){gyro.stopTracking()}})(window);(function(t){(t.rc=t.rc||{}).events={BUTTON_PRESS:"rc_button_press",BUTTON_RELEASE:"rc_button_release",TRACKPAD_PRESS:"rc_trackpad_press",TRACKPAD_MOVE:"rc_trackpad_move",TRACKPAD_RELEASE:"rc_trackpad_release",ACCELEROMETER:"rc_accelerometer"}})(window);
...
m.style.position="absolute";m.style.top=c+"px";m.style.left=e+"px";m.style.width=h+"px";m.style.height=r+"px";m.className="rc-trackpad";var n=function(a,b){relX=a-e;relY=b-c;return{x:relX*k/h,y:relY*p/r}},q=!1,s=function(c){c.preventDefault();q=!0;c=b?c.changedTouches[0]:c;c=n(c.pageX,c.pageY);g.send(g.events.TRACKPAD_PRESS,{name:a,x:c.x,y:c.y})},u=function(c){c.preventDefault();q&&(c=b?c.changedTouches[0]:c,c=n(c.pageX,c.pageY),g.send(g.events.TRACKPAD_MOVE,{name:a,x:c.x,y:c.y}))},t=function(c){c.preventDefault();
q=!1;c=b?c.changedTouches[0]:c;c=n(c.pageX,c.pageY);g.send(g.events.TRACKPAD_RELEASE,{name:a,x:c.x,y:c.y})};b?(f(m,"touchstart",s),f(m,"touchmove",u),f(m,"touchend",t)):(f(m,"mousedown",s),f(m,"mousemove",u),f(m,"mouseup",t));return m}}})(window,document);